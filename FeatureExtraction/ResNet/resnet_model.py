...
HParams = namedtuple('HParams',
                     'batch_size, num_classes , is_gray,num_feature, min_lrn_rate, lrn_rate,'
...
class ResNet(object):
  ...
  def __init__(self, hps, image_size, mode,is_gray=True):
    """ResNet constructor.
    ...
    """
    if is_gray:self.n_channels = 1
    else:self.n_channels = 3
    self.hps = hps
    self._images = tf.placeholder(dtype=tf.float32,shape=[hps.batch_size,image_size,image_size,self.n_channels],name="images")
    self.labels = tf.placeholder(dtype=tf.float32,shape=[hps.batch_size,hps.num_classes],name="labels")
#     self._images = images
#     self.labels = labels
    self.mode = mode
  ...
  def _build_model(self):
    with tf.variable_scope('init'):
      x = self._images
      """第一层卷积（3,3x3/1,16）"""
      x = self._conv('init_conv', x, 3, self.n_channels, 16, self._stride_arr(1))      
    # 残差网络参数
    strides = [1, 2, 2]
    # 激活前置
    ...
    with tf.variable_scope('logit'):
      logits = self._fully_features_connected(x, self.hps.num_classes)
      self.predictions = tf.nn.softmax(logits)
    # 全连接层 + Softmax
#     with tf.variable_scope('logit'):
#       self.feature_vcts = self._fully_features_connected(x, self.hps.num_feature)
#       logits = self._fully_connected(self.feature_vcts, self.hps.num_classes)
#       self.predictions = tf.nn.softmax(logits)
  ...
  def _fully_connected(self, x, out_dim):
    ...
    w = tf.get_variable('DW1', [x.get_shape()[1], out_dim],
                        initializer=tf.uniform_unit_scaling_initializer(factor=1.0))
    # 参数b，0值初始化
    b = tf.get_variable('biases1', [out_dim], initializer=tf.constant_initializer())
    # 计算x*w+b
    return tf.nn.xw_plus_b(x, w, b)

  def _fully_features_connected(self, x, out_dim):
    # 输入转换成2D tensor，尺寸为[N,-1]
    self.x_feature = tf.reshape(x, [self.hps.batch_size, -1])
    # 参数w，平均随机初始化，[-sqrt(3/dim), sqrt(3/dim)]*factor
    w = tf.get_variable('DW', [self.x_feature.get_shape()[1], out_dim],
                        initializer=tf.uniform_unit_scaling_initializer(factor=1.0))
    # 参数b，0值初始化
    b = tf.get_variable('biases', [out_dim], initializer=tf.constant_initializer())
    # 计算x*w+b
    res = tf.nn.xw_plus_b(self.x_feature, w, b)
    return res
